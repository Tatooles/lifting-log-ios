...
"""),
    ...
    func testForegroundAndManualRetryUseSharedAuthenticationRecovery() throws {
        let appSource = try sourceFileContents("Baros/App/BarosApp.swift")
        let shellSource = try sourceFileContents("Baros/App/AppShellView.swift")
        let settingsSource = try sourceFileContents("Baros/Features/Profile/SettingsAccountSection.swift")
    ...
    func testAuthenticatedConvexOwnerMismatchHidesLocalDataAndClearsAuth() throws {
        let appSource = try sourceFileContents("Baros/App/BarosApp.swift")
        let coordinatorSource = try sourceFileContents("Baros/Core/Sync/SyncRecoveryCoordinator.swift")
